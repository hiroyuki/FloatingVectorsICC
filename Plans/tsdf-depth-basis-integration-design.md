...
> **状態（2026-07-09 追記）**: フェーズ1a＋1b（active-block clear）が **main にマージ済み**。
> `TSDFIntegrator.useDepthBasis` の**コード既定は `false` のまま**だが、**本番シーン `Assets/main.unity`
> は `useDepthBasis: 1` を serialize しており、実運用は depth 基準で動作している**（RetainGhost /
> `clearVolumeOnNewBatch` 経路のみ有効。それ以外は voxel 基準へフォールバックし警告ログを出す）。
> 「既定 OFF で main 影響なし」という以下の記述はコード既定についての話で、**本番シーンの挙動とは異なる**点に注意。
> 未着手はフェーズ2（Average+Fold の depth 基準化 = thin-limb 経路）。
...
- ~~既定を depth 基準に切替えるのは本番モードでの品質承認後（現状は `useDepthBasis` 既定 OFF）。~~
  **→ 済（2026-07-09）: 本番シーン `main.unity` で `useDepthBasis: 1` を適用済み**（コード既定 `false` は据え置き。冒頭の状態注記参照）。
