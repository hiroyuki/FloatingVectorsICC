...
> **進捗（2026-07-09 追記）**: §4 の各オプションの現況:
> - **①** `edgeRejectRadius` → **0 にする方針で確定**（別セッションで適用予定。§4 表の注記参照）。
> - **④ depth 基準**（本命の大改造）は「将来」ではなく **実装・有効化済み**。フェーズ1a＋1b が main
>   にマージされ、**本番シーン `main.unity` は `useDepthBasis: 1`**（実測 voxel 基準比 **2.75×**、
>   §3 の RetainGhost / free-space 制約は投影ゲートで担保）。設計と実測は
>   `Plans/tsdf-depth-basis-integration-design.md` 参照。残るはフェーズ2（Average+Fold の depth 基準化）。
> - よって以下の「まず②を安全策に、④は将来」という推奨順序は **④が既に入った現況に合わせて読み替えること**。
>   ②（フラスタム AABB 制限）は depth 基準と直交する追加最適化として依然有効。
...
> **決定（別セッションで実施予定）**: ① は **`edgeRejectRadius` を 0 にする方針で確定**。A/B の再検討は不要。§5-2 の測定ステップは「0 適用の確認」に読み替える。
