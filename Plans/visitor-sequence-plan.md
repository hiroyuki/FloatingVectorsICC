...
1. **Attract** — ゴースト再生（v11s bodies）。presence 検知で開始
...
|---|---|---|
| Live（Calibrate/FreeMove/Shoot） | 融合 bodies（useExternalBodies） | submit ON |
...
再生（無ければ k4abt 再解析）/ capture 失敗 → 謝罪表示 → Attract / アップロード失敗 →
