...
作成 2026-07-02 / 改訂 2026-07-02 (Codex review round 1–3 反映; 分類を表面距離 d−radius へ、
体格キャリブは N2H 化)
...
- **分類ルール（点→ボーン）**: 各ボーンで中心線距離 `d`（point-to-segment）と半径
  `radius`（テーパーで t に沿い `lerp(radiusA,radiusB,t)`）から表面距離 `surf = d − radius`。
  割当は **`rank = max(surf, 0)` 最小のボーン**。**内側重複（`rank==0` が複数、例: 肩＝
  上腕根が胴カプセル内側）は中心線距離 `d` 最小で tie-break**、さらに並べば bone index で
  決定的に。→「1つの内側=それ」「複数内側=中心線が近い方（肩なら上腕）」「外側=最近傍表面」
  が全部正しくなる。背景カリング = 全ボーン最小 `surf > surfaceMargin`。
  - 単純な符号付き `surf` 最小（most-negative wins）は内側重複で太いボーンが薄いボーンの
    表面点を奪う（Codex 指摘）。`abs(surf)` 最小は逆に胴奥の点を細い腕に奪われる。よって
    **rank クランプ＋中心線 tie-break** を採る。
  - 正規化 `d/radius` は太い部位が細い部位近傍の点を過剰取得するため不採用。精度不要なので
    **異方性カプセル（関節 orientation 断面）も不採用**、等方半径＋端点テーパーで十分。
  - 実装注意（Codex）: GPU 上で `rank==0` は**厳密 float 等価でなく epsilon 判定**にし、
    内側重複の中心線 tie-break を安定化する。
- **半径は毎フレームのキャリブ（体格オートスケール）を廃し、標準半径テーブル＋全体スケール
  1本（Inspector）で固定**。体格オートスケールは将来の N2H（下記）。
...
| 分類 + パラメータ化 | 新規 compute | seed→ボーン割当（`rank=max(d−radius,0)` 最小＋内側は中心線 `d` tie-break, `PointCloudCapsuleFilter` の point-to-segment 数式流用）→ `(t,cv,cw)` |
...
- **標準半径テーブル**（`boneRadius[_BoneCount]`、**絶対値 m で固定**。例 胴~0.18 / 上腕~0.06
  / 前腕~0.045 / 太もも~0.09 / すね~0.05）＋**端点テーパー**（両端で太さ可変）。
  **全体スケール1本**（`radiusScale`、Inspector）で来場者の体格差を手動吸収（毎フレーム計算なし）。
- パラメータ定義: `seedCount`（例 2k–20k）、`trailDuration`（例 0.4–1.5s）、
  `historySamples K`（例 8–16）、`radiusScale`（既定 1.0）、`surfaceMargin`
  （例 0.03m、`d − radius > surfaceMargin` の seed は背景として破棄）、`smoothSubdiv`（Phase3）。

### N2H（今後・不採用でスコープ外、将来オプション）
- **体格オートスケール・キャリブ**: 起動時に一度、肩幅 or 背骨長（`SPINE_NAVEL`〜`NECK`）を
  `TryGetJointWorld` で測り `radiusScale` を自動決定（毎フレームではなく1回）。手軽版で
  足りない体格差が問題化したら足す。分類メトリクス（表面距離）自体は変えずスケールだけ動かす。
...
- seed 選択: index stride/hash ＋ validity ＋ `min surf > surfaceMargin` 破棄 ＋ sanity。
- 分類 compute: seed→ボーン割当（`rank=max(surf,0)` 最小、内側重複は中心線 `d` tie-break）→ `(t,cv,cw)`。
...
- `Assets/Scripts/PointCloud/Resources/MotionCurvesBuild.compute`（新規）— sanity カリング＋
  分類（`rank=max(d−radius,0)`＋中心線 tie-break）＋パラメータ化＋履歴再投影＋（Phase3）Catmull-Rom→頂点生成。
