...
| **Space** | live: カウントダウン（既定 5 秒、画面中央に大表示）→ freeze。**Rec 中でも有効**（録画は継続、描画/TSDF/BT だけ静止）。**Rec 中に freeze すると既定 3 秒後に録画が自動停止**（`stopRecAfterFreezeSeconds`、0=無効。freeze 解除で取り消し、freeze 自体は残る）。再度 Space で解除は即時、カウントダウン中の Space はキャンセル。playback 中は従来通り即 pause |
