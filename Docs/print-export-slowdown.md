...
> **全項目 解決済み (2026-07-21)**。以下は当時の TODO と、その後の実測結果。

1. ~~**`main.unity` を pull 後、3フラグが `0` になっているか**~~
   → 確認済み。`stlContactBeads` / `stlCullInteriorFaces` はコードから削除、
   `stlCurveSupportPillars` は現行 Mac シーンで `False`（reflection 確認）。
2. ~~Mac の 540 MB 3MF の正体~~
   → **再現せず**。現行コードで `_final.3mf` は 20.1 MB（上「## 未対応」参照）。
   原因は撤去済み O(n²) 機能で確定。
3. ~~`playbackFolderPathMacOverride` が別テイク `2026-07-14_13-33-44` を指す~~
   → 現在は Windows と同じ **`2026-07-14_15-09-52`** を指す（解消済み）。
...
- [x] スライサ（Bambu Studio）での目視確認 — **完了 (2026-07-21)**。
  `_final.3mf`（約 37 MB）をスライサに読み込んで目視 OK。
- [x] Mac 側 540 MB の原因特定 — **再現せず・原因確定 (2026-07-21)**。
  現行コード (`70274c9`) で Mac 実測: 同テイク `2026-07-14_15-09-52` /
  `SeekToFrame(1937)` / preroll 75s / 決定稿設定のまま、
  `ExportObj` **9.9s → 120 MB OBJ (1,792,617 tris)** → `fix_body.py` →
  `_final.3mf` **20.1 MB**（MeshFix 78,990v/127,261f → 82,110v watertight, 25s）。
  ハングも 540 MB 爆発も無し（540 MB の 1/27）。`stlContactBeads` /
  `stlCullInteriorFaces` フィールドは現行コードから消えており（reflection で不在確認）、
  **540 MB を生む O(n²) 経路は構造的に存在しない**。旧 Mac の 540 MB は
  これら撤去済み機能が原因だったと確定。
