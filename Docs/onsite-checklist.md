...
- [ ] ExperienceConfig アセット作成（`Create > FloatingVectors > Experience Config`）: rigSerialOrder（**どのカメラを 1 番にするか = +X/選択球の並ぶ向き**をここで確定）、attractRecordingRoot（現地収録のテイク群）、`dryRunPublish=false`
...
- [ ] （小・任意）DwellSphere.showVisualization=false はリング/芯を隠さない（Codex non-blocking 2026-07-13。director は GO ごと無効化するので運用上は無害）
