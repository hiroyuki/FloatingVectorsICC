...
namespace PointCloud
{
    public static class PointCloudRecording
    {
        ...
        /// <summary>
        /// Writes <c>calibration/floor.yaml</c> with both the operator-tuned floor
        /// height and the floor-levelling correction Pose (tilt + height that makes
        /// the picked floor horizontal at y=0). The Pose is stored as raw floats so
        /// this recording layer stays free of UnityEngine types: <paramref
        /// name="levelPos"/> is the translation [x,y,z] and <paramref
        /// name="levelRot"/> the rotation quaternion [x,y,z,w]. Identity levelling
        /// (pos 0, rot 0,0,0,1) is written too so the file always round-trips.
        /// </summary>
        public static void WriteFloor(string rootDir, float floorY, float[] levelPos, float[] levelRot)
        {
            if (levelPos == null || levelPos.Length != 3) levelPos = new[] { 0f, 0f, 0f };
            if (levelRot == null || levelRot.Length != 4) levelRot = new[] { 0f, 0f, 0f, 1f };
            Directory.CreateDirectory(CalibrationDir(rootDir));
            string path = Path.Combine(CalibrationDir(rootDir), "floor.yaml");
            var sb = new StringBuilder();
            sb.Append("# Generated by FloatingVectorsICC. Operator-tuned floor for this room/rig:\n");
            sb.Append("# rebase_floor_y = calibration-frame Y that becomes world y=0.\n");
            sb.Append("# floor_leveling_* = world-space tilt+height correction (from the\n");
            sb.Append("# 3-point floor pick) applied AFTER the camera rebase. rot is a\n");
            sb.Append("# quaternion [x,y,z,w]; identity = no tilt.\n");
            sb.Append($"rebase_floor_y: {F(floorY)}\n");
            sb.Append($"floor_leveling_pos: [{F(levelPos[0])}, {F(levelPos[1])}, {F(levelPos[2])}]\n");
            sb.Append($"floor_leveling_rot: [{F(levelRot[0])}, {F(levelRot[1])}, {F(levelRot[2])}, {F(levelRot[3])}]\n");
            File.WriteAllText(path, sb.ToString(), new UTF8Encoding(false));
        }

        /// <summary>
        /// Reads <c>calibration/floor.yaml</c> including the levelling Pose. Missing
        /// levelling keys parse as identity (back-compat with files that predate the
        /// floor-plane pick). Returns false only when the file is missing / has no
        /// usable field; a present-but-invalid levelling degrades to identity
        /// rather than failing the whole read.
        /// </summary>
        public static bool TryReadFloor(string rootDir, out float floorY,
                                        out float[] levelPos, out float[] levelRot)
        {
            floorY = 0f;
            levelPos = new[] { 0f, 0f, 0f };
            levelRot = new[] { 0f, 0f, 0f, 1f };
            if (string.IsNullOrEmpty(rootDir)) return false;
            string path = Path.Combine(CalibrationDir(rootDir), "floor.yaml");
            if (!File.Exists(path)) return false;
            try
            {
                ...
                bool any = false;
                if (root.Children.TryGetValue(new YamlScalarNode("rebase_floor_y"), out var v)
                    && v is YamlScalarNode s
                    && float.TryParse(s.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out floorY)
                    && float.IsFinite(floorY) && Math.Abs(floorY) < 100f)
                {
                    any = true;
                }
                else floorY = 0f;

                if (TryReadFloatSeq(root, "floor_leveling_pos", 3, out var pos)
                    && TryReadFloatSeq(root, "floor_leveling_rot", 4, out var rot)
                    && IsFiniteArr(pos) && IsFiniteArr(rot)
                    && Math.Abs(pos[0]) < 100f && Math.Abs(pos[1]) < 100f && Math.Abs(pos[2]) < 100f
                    && (rot[0] * rot[0] + rot[1] * rot[1] + rot[2] * rot[2] + rot[3] * rot[3]) > 1e-6f)
                {
                    levelPos = pos;
                    levelRot = rot;
                    any = true;
        ...
        /// <summary>
        /// Writes <c>calibration/sensing_area.yaml</c> — the sensing box fitted to
        /// THIS rig. Machine-local on purpose: the two exhibition sets share a scene
        /// but stand their cameras in different places, so a box serialised into
        /// main.unity would arrive at the other set wrong (and silently, since a
        /// plausible-looking box is hard to spot). Same reasoning as cameras.yaml.
        /// The inset / height the fit was made with travel along so the file
        /// documents how it was produced and a re-fit can reproduce it.
        /// </summary>
        public static void WriteSensingArea(string rootDir, float[] center, float[] size,
                                            float insetMeters, float areaHeight)
        {
            if (center == null || center.Length != 3) center = new[] { 0f, 0f, 0f };
            if (size == null || size.Length != 3) size = new[] { 0f, 0f, 0f };
            Directory.CreateDirectory(CalibrationDir(rootDir));
            string path = Path.Combine(CalibrationDir(rootDir), "sensing_area.yaml");
            var sb = new StringBuilder();
            sb.Append("# Generated by FloatingVectorsICC. Sensing area (BoundingVolume)\n");
            sb.Append("# fitted to THIS machine's camera rig — do not copy between sets.\n");
            sb.Append("# center/size are world metres; rotation is always identity.\n");
            sb.Append("# inset_meters = clearance kept from each camera to the nearest face.\n");
            sb.Append($"center: [{F(center[0])}, {F(center[1])}, {F(center[2])}]\n");
            sb.Append($"size: [{F(size[0])}, {F(size[1])}, {F(size[2])}]\n");
            sb.Append($"inset_meters: {F(insetMeters)}\n");
            sb.Append($"area_height: {F(areaHeight)}\n");
            File.WriteAllText(path, sb.ToString(), new UTF8Encoding(false));
        }

        /// <summary>
        /// Reads <c>calibration/sensing_area.yaml</c>. Returns false when the file is
        /// missing or either vector is unusable, in which case the caller keeps
        /// whatever the scene authored — see SensorManager.ApplySensingAreaFromCalibration.
        /// </summary>
        public static bool TryReadSensingArea(string rootDir, out float[] center, out float[] size)
        {
            center = new[] { 0f, 0f, 0f };
            size = new[] { 0f, 0f, 0f };
            if (string.IsNullOrEmpty(rootDir)) return false;
            ...
            try
            {
                ...
                if (stream.Documents.Count == 0) return false;
                if (!(stream.Documents[0].RootNode is YamlMappingNode root)) return false;
                if (!TryReadFloatSeq(root, "center", 3, out var c)) return false;
                if (!TryReadFloatSeq(root, "size", 3, out var sz)) return false;
                if (!IsFiniteArr(c) || !IsFiniteArr(sz)) return false;
                // A degenerate or absurd box would blank the installation silently.
                for (int i = 0; i < 3; i++)
                {
                    if (Math.Abs(c[i]) > 100f) return false;
                    if (sz[i] < 0.1f || sz[i] > 100f) return false;
                }
                center = c;
                size = sz;
                return true;
        ...
        private static bool TryReadFloatSeq(YamlMappingNode root, string key, int len, out float[] arr)
        {
            arr = new float[len];
            if (!root.Children.TryGetValue(new YamlScalarNode(key), out var node)) return false;
            if (!(node is YamlSequenceNode seq) || seq.Children.Count != len) return false;
            for (int i = 0; i < len; i++)
            {
                if (!(seq.Children[i] is YamlScalarNode sc)) return false;
                if (!float.TryParse(sc.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out arr[i]))
                    return false;
            }
            return true;
        }

        private static bool IsFiniteArr(float[] a)
        {
            for (int i = 0; i < a.Length; i++)
                if (!float.IsFinite(a[i])) return false;
            return true;
        }
