...
**成果物**: `eval/INSTALL_cuda13.md`(ダウンロードリンク・チェックサム記録欄付きインストール指示書)
...
2. **cuDNN 9.x for CUDA 12**(NVIDIA 公式からのみ。zip 展開 → bin の DLL 群を PATH に通すか、
   ...
   - **zip の SHA-256 を取得元ページの値と照合し、`eval/INSTALL_cuda13.md` に記録**
...
## Phase 4 前提変更 — cu13 統一(2026-07-19)

本番機の GPU が **RTX 5080(Blackwell / sm_120)** と判明。ORT 公式 cu12 ビルドは
sm_120 カーネル未同梱で `cudaErrorInvalidPtx` の報告あり(onnxruntime#26177 未解決)のため、
**開発機・本番機とも CUDA 13 系に統一**する(4070 も CUDA 13 サポート内)。

- native DLL の取得元を NuGet Gpu.Windows(cu12)から
  **GitHub リリース `onnxruntime-win-x64-gpu_cuda13-1.26.0.zip`** に変更(managed 層 1.26.0 は不変)
- インストール指示書は `eval/INSTALL_cuda13.md` に改訂(旧 INSTALL_cuda12.md を置換)
- cu12 → cu13 はビルドフレーバー変更なので、Phase 3 追補 2 と同じオフライン A/B を
  1 テイク実施し、ズレていれば v11s リファレンスを **5080 機の cu13 で再生成**してから運用
- 本番機 DLL 復元済み(2026-07-19)。CUDA Toolkit **13.3** + cuDNN **9.24.0(cu13)** インストール済み
- **本番機(5080)スモーク合格(2026-07-19)**: `EP=Cuda (requested=Cuda)`、ctor 2.0s、
  Pose 初回 1082ms(ウォームアップ)→ 2回目 19ms、Detect 実行成功。InvalidPtx 出ず
  — cu13 ビルドは Blackwell でネイティブ動作
- 注意: CUDA 13 から DLL は `bin\x64` 配下(PATH は bin\x64 を登録済み)。
  インストール前から起動していたプロセス(Unity Hub 等)は旧 PATH を持つので再起動が必要
- **オフライン A/B 完了(2026-07-19、rec2_turn 909 frames / fused 129 emits)**:
  cu13 の CPU 実行 vs CUDA 実行 = **全関節 p50 0.0mm / p95 ≤2.9mm**、
  境界フレームの argmax 反転が数フレーム(head max 269mm / wrist max 65mm)
  — Phase 3 追補 2 で確立した「フレーバー差の性格」と同じで、恒常的なズレなし。
  結果: `eval/results/offline_ab_20260719_155348.md`
- 帰結: cu13 を本番フレーバーとして確定
- **cu12 リファレンスとの直接 A/B(15-50-24 全 1816 frames、2026-07-19)**:
  p50 全関節 0.0mm / p95 ≤5.6mm、腕の数フレームのみ max 344mm・手首 accel 30.3k→37.9k
  — Phase 3 追補 2 の cu12 移行時と同一の性格・規模。cu13 固有の劣化なし
  (`eval/results/offline_ab_20260719_161849.md`)
- **v11s リファレンス cu13 再生成 完了(2026-07-19、5080 機)**: 生テイクは
  `D:\FloatingVectorsICC\RecordingBase` → `D:\Dropbox\projects\ICC\Recordings\RecordingBase`
  のジャンクション経由。出力 `D:\RecordingBaseV11s_cu13`(32 take、41 分、failed=0。
  color 無し短テイク 7 つは旧生成時と同じ理由でスキップ)。旧 cu12 リファレンスは
  `D:\RecordingBaseV11s` に温存。以後の検証は cu13 リファレンスを使う
- 残タスク: 開発機(4070)への CUDA 13.3 + cuDNN 9.24(cu13) サイドバイサイド導入と
  DLL 差し替え(`eval/INSTALL_cuda13.md`)。急ぎではないが、開発機で骨格検証をする前までに
