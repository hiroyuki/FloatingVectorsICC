...
- **シャッターの訂正（ユーザー指摘）**: ToF 深度の露光は固定で上げられない。上げられるのは**カラー**のみ。
  理屈も整合: カクつき（追いつきジャンプ）の根源は RTMPose が見る**カラーのブラー**だった。
  深度側の羽毛状の広がりは高速面の深度ノイズ/フライングピクセル（露光ブラーではない）
- **次回収録 TODO（確定）**: ① カラー露光（シャッター）を上げる ② trigger2ImageDelayUs を
  カメラ毎 160µs×n でスタガ ③ 三脚等をキャプチャボリューム外へ
- **深度サブフレーム補間は却下**（重い・品質未知数、ユーザー同意）
- 検討中: **速度連動トレイル減衰**（案3）— 最寄り関節速度（PointCloudJointMotionField の速度場を流用）を
  TSDF 積分重みに掛け、速い部位は残像を残さずカーブに軌跡を任せる。「遅=面の彫刻/速=線」の自動分担。
  実装軽め、ただし表現の変化なのでユーザーのルック判断待ち
