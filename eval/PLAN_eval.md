...
- (h) **TODO（live-capture 統合 / main マージ計画に含める）**:
  ...
  2. **プロジェクタ写り込みマスク — 実装済み（0f35788, ユーザー承認済）**: `ProjectorMask`（新規）が
     extrinsics から他カメラ位置を各カメラ深度画像へ投影し、半径 45px（320幅基準）の深度を無効化。
     **IR 輝度ゲート付き**（可視時 5x5平均 ~29000 / 遮蔽時 ~50 → 閾値 2000）: ダンサーがプロジェクタを
     遮った瞬間はマスク解除され体のピクセルは失われない（frame10 で実証）。SensorRecorder.EmitFrameAt で
     全消費者（mesh/TSDF/BT/event）より前に適用、Inspector トグル maskRivalProjectors（既定 ON）。
     **検証済**: 末尾空舞台の TSDF 中央スペックル柱が ON で消滅（tail_maskOFF/ON.png）。
