...
- (h) **TODO（live-capture 統合 / main マージ計画に含める）**:
  ...
  2. **プロジェクタ写り込みマスク（恒久対策・提案中）**: extrinsics から他カメラ位置を各カメラの
     深度画像へ投影し、周辺 ~15px の深度を無効化する自動マスク。スタガでも輝点自体は残るため、
     録画資産（15-50-24 含む）と真正対配置の恒久対策になる。TSDF/curve/mesh 全消費者に効く
     深度ソース側で適用
